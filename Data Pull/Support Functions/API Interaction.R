...
timeStamp <- function(storeIn, basepath=".", goalDate = lubridate::now()) {
...
storeParquet <- function(obj, source, storeIn, basepath='.', tolerance=24) {
  ...
  if (nrow(mostRecent) == 0){
    ...
    writepath <- file.path(basepath, storeIn, name)
    ...
    proximity <- timeStamp(storeIn, basepath)$`Record History` %>%
...
runIfExpired <- function(source, storeIn, f, tolerance = 24, return_recent = FALSE, basepath = "Data Pull") {
  source <- "rsv-net"
  storeIn <- "RESP NET Archive"
  basepath <- "Data Pull"
  ...
  mostRecent <- timeStamp(storeIn, basepath)$`Report Relative to Date` %>%
