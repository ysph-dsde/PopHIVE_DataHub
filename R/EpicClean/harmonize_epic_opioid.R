...
#EPIC ED all cause
epic_ed_all_latest_file = datetimeStamp( basepath='./Data/Archive/Cosmos ED/Youth Well-being/Obesity')$`Report Relative to Date` %>%
  filter(Delta==min(Delta)) %>%
  filter(grepl('AllPts', filePath)) %>%
  pull(filePath) 

epic_ed_all <- epic_age_import_youth(ds_name = paste0('./Data/Archive/Cosmos ED/Youth Well-being/Obesity/',epic_ed_all_latest_file)) %>%
  rename(N_ED_epic_pts = N_cases_epic) %>%
  dplyr::select(geography, Level, date, N_ED_epic_pts)
...
epic_ed_combo <- bind_rows(epic_ed_rsv, epic_ed_flu , epic_ed_covid) %>%
  ...
  mutate(
    ...
    pct_ED_epic = N_ED_type / N_ED_epic_pts * 100,
