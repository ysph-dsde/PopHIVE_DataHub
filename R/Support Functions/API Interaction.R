...
storeFile <- function(pulledData, sourceName, storeIn, fileType = "parquet", 
                      includeTime = TRUE, stampBeginning = TRUE, allowNewDir = FALSE,
                      tolerance = 24, basepath = "Data Pull", timeZone = "EST") {
  ...
  #                   the date.
  #    "allowNewDir": if the destined directory for the archive is not present,
  #                   then allow the user to control of one is autogenerated or not.
  ...
  if (!dir.exists(fullpath)){
    ...
    if (allowNewDir == TRUE){
      message(sprintf("Path '%s' doesn't exist, or isn't a directory.", fullpath))
    ...
    } else if (allowNewDir == FALSE){
      stop(sprintf("Path '%s' doesn't exist, or isn't a directory. Make the file and rerun the script.", fullpath))
    }
  }
...
## change name to PopHIVE Data Hub
## Integrate files into Dan's structure
## Plot code in the new repo PopHIVE Shiny repo
## Add a new directory for the troubleshooting - gitignore this directory and have it in a separate branch


runIfExpired <- function(sourceName, storeIn, f, fileType = "parquet", returnRecent = TRUE,
                         includeTime = TRUE, stampBeginning = TRUE, allowNewDir = FALSE,
                         tolerance = (24*7), basepath = "Data Pull", timeZone = "EST") {
  #     "sourceName": the name of the source. Recommend using standard naming
  #                   schemes to better manage archive. One "source" for one API.
  ...
  if(any(mostRecent == "No History") | length(mostRecent) != 2) {
    ...
    storeFile(pulledData, sourceName = sourceName, storeIn = storeIn, fileType = fileType, 
              includeTime = includeTime, stampBeginning = stampBeginning, allowNewDir = allowNewDir,
