...
lapply(list.files('./R/Data Pull/', full.names = T), function(X) {
  print(X)
  tryCatch(source(X), error = function(e) {
    message("failed ", X)
    print(e)
  })
})
...
pop <- read.csv(
  "./Data/other_data/Single-Race Population Estimates 2020-2023 by State and Single-Year Age.csv"
) %>%
  dplyr::rename(
...
national_popwgted_avg_rsv <- combined_file_rsv %>%
  ...
  filter(
    !geography %in%
      c("United States", "Armed Forces Africa", "None of the above")
  ) %>%
  left_join(pop_unstra %>% dplyr::select(-age_level), by = "geography") %>%
  group_by(date, outcome_label1) %>%
  mutate(
    wgt = ifelse(
      is.na(Outcome_value1),
      NA,
      popsize / sum(popsize[!is.na(Outcome_value1)])
    )
  ) %>% # some states are missing (ED/ww) data for some dates, thus they are not included in the calculation
  mutate(across(
    c(starts_with("Outcome_value1")),
    ~ sum(.x * wgt, na.rm = T)
    # .names = "{.col}_wgtavg")
  )) %>%
  mutate(across(
  ...
  dplyr::select(-popsize, -wgt, -geography) %>%
  distinct() %>%
...
combined_file_rsv_addavg <- combined_file_rsv %>%
  ...
  mutate(
    geo_strata = ifelse(
      geography == "United States" & geo_strata != "national_pop_wgted_avg",
      "national",
      geo_strata
    )
  ) %>%
...
combined_file_rsv <- cbind.data.frame(
  combined_file_rsv,
  dates2[, c('MMWRyear', 'MMWRweek')]
) %>%
  mutate(
    epiyr = MMWRyear,
...
write.csv(
  combined_file_rsv,
  './Data/Plot Files/Comparisons/rsv_combined_all_outcomes_state.csv'
)
...
combined_file_flu <- cbind.data.frame(
  combined_file_flu,
  dates2[, c('MMWRyear', 'MMWRweek')]
) %>%
  mutate(
...
write.csv(
  combined_file_flu,
  './Data/Plot Files/Comparisons/flu_combined_all_outcomes_state.csv'
)
...
combined_file_covid <- cbind.data.frame(
  combined_file_covid,
  dates2[, c('MMWRyear', 'MMWRweek')]
) %>%
  mutate(
...
write.csv(
  combined_file_covid,
  './Data/Plot Files/Comparisons/covid_combined_all_outcomes_state.csv'
)
...
write.csv(
  d1_all,
  './Data/Plot Files/Cosmos ED/rsv_flu_covid_county_filled_map_nssp.csv'
)
...
b2019 <- read.csv(
  './Data/Pulled Data/pneumococcus/jiac058_suppl_supplementary_table_s2.csv'
) %>%
  group_by(State, sero) %>%
  summarize(N_cases = n()) %>%
...
rsv_nrevss_test <- read.csv(
  "./Data/Plot Files/NREVSS/rsv_ts_nrevss_test_rsv.csv"
) %>%
  dplyr::select(-X)
...
rsv_nrevss_test_avg <- rsv_nrevss_test %>%
  ...
  dplyr::select(-popsize, -wgt, -level, -x, ) %>%
  distinct() %>%
...
write.csv(
  rsv_nrevss_test,
  "./Data/Plot Files/NREVSS/rsv_ts_nrevss_test_rsv_nat.csv",
  row.names = F
)
...
rsv_respnet_hosp_age_avg <- rsv_respnet_hosp_age %>%
  ...
  dplyr::select(-popsize, -wgt, -state) %>%
  distinct() %>%
  ...
  mutate(
    hosp_rate_3m = zoo::rollapplyr(hosp_rate, 3, mean, partial = T, na.rm = T),
...
write_parquet(
  g1_metro,
  './Data/Webslim/respiratory_diseases/rsv/google_dma.parquet'
)
#
