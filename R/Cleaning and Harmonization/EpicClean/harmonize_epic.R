...
#EPIC ED all cause
epic_ed_all_latest_file = datetimeStamp(storeIn = 'All visits', basepath='./Data/Pulled Data/Cosmos ED')$`Report Relative to Date` %>%
...
#EPIC ED RSV
epic_ed_rsv_latest_file = datetimeStamp(storeIn = 'rsv', basepath='./Data/Pulled Data/Cosmos ED')$`Report Relative to Date` %>%
  filter(Delta==min(Delta)) %>%
  pull(filePath)
...
# EPIC ED FLU
epic_ed_flu_latest_file = datetimeStamp( storeIn = 'flu', basepath='./Data/Pulled Data/Cosmos ED')$`Report Relative to Date` %>%
  filter(Delta==min(Delta)) %>%
  pull(filePath)
...
# EPIC ED COVID
epic_ed_covid_latest_file = datetimeStamp( storeIn = 'covid', basepath='./Data/Pulled Data/Cosmos ED')$`Report Relative to Date` %>%
  filter(Delta==min(Delta)) %>%
  pull(filePath)
